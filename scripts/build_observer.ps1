...
$outputPath = Join-Path $Workspace "mql5\StraddleObserver.ex5"
$logPath = Join-Path $Workspace "artifacts\observer-compile.log"
...
if (-not (Test-Path -LiteralPath $sourcePath -PathType Leaf)) {
    throw "Observer source was not found at '$sourcePath'."
}
...
if ($compileLog -notmatch "Result:\s+0 errors,\s+0 warnings") {
    throw "Observer compilation failed. Review '$logPath'."
}
...
Write-Host "Compiled read-only observer: $($artifact.FullName)"
