...
$sourcePath = Join-Path $Workspace "mql5\StraddleReplica.mq5"
$outputPath = Join-Path $Workspace "mql5\StraddleReplica.ex5"
$logPath = Join-Path $Workspace "artifacts\compile.log"
...
if (-not (Test-Path -LiteralPath $sourcePath -PathType Leaf)) {
    throw "EA source was not found at '$sourcePath'."
}
...
if (-not (Test-Path -LiteralPath $logPath -PathType Leaf)) {
    throw "MetaEditor did not produce the compile log '$logPath' (exit code $($process.ExitCode))."
}

$compileLog = Get-Content -LiteralPath $logPath -Raw
if ($compileLog -notmatch "Result:\s+0 errors,\s+0 warnings") {
    throw "EA compilation failed. Review '$logPath'."
}
...
Write-Host "Compiled StraddleReplica: $($artifact.FullName) ($($artifact.Length) bytes)"
